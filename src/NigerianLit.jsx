...
const books = [
  ...
  { id: 113, title: "The Second Chance", author: "Nyengi Koin", year: 1986, genre: "Novel", description: "An Egret romance and thriller about love, class pressure, marriage, and the possibility of another beginning.", synopsis: "The Second Chance follows Mina, a wealthy young woman from Ikoyi, Lagos, whose love for Richard crosses the class boundary her mother cannot accept. Mina marries him despite family pressure and social judgment, but compromise, pride, and the realities of marriage begin to test the relationship. Nyengi Koin's novel belongs to the Egret romance and thrillers series, a memorable strand of Nigerian popular fiction from the 1980s.", cover: "", openLibrary: "https://openlibrary.org/works/OL5066441W/The_second_chance", buyBooks: "https://buybooks.ng/product/second-chance-by-nyengi-koin/", tags: ["Novel", "Nigerian Fiction", "Romance", "Lagos", "School Classic"] },
  { id: 114, title: "The Beggars' Strike", author: "Aminata Sow Fall", year: 1979, genre: "Novel", description: "A sharp Senegalese satire in which the beggars of Dakar organise after the authorities try to remove them from the city.", synopsis: "The Beggars' Strike, originally published in French as La Greve des battu, imagines a city administration trying to cleanse Dakar's streets of beggars for the sake of order, respectability, and tourism. The policy backfires when the beggars withdraw their prayers, blessings, and social presence, exposing the hypocrisy of officials who despise the poor while depending on them spiritually and politically. Aminata Sow Fall's compact satire became a widely taught West African text about power, poverty, dignity, and public morality.", cover: "", openLibrary: "https://openlibrary.org/works/OL3075656W/The_Beggars%27_Strike", readableUrl: "https://archive.org/details/beggarsstrikeord0000fall", tags: ["Novel", "West African", "Senegal", "Satire", "Political"] },
];
...
const SOUND_KEY = "nlitSound_v1";
const MUSIC_KEY = "nlitMusic_v1";
...
function writePref(key, value) {
  try {
    ...
    // Preference writes are optional.
  }
}
...
function DetailModal({ book, onClose, saved, onToggleSaved }) {
  ...
  const nigeriaBuyLink =
    ...
    book.africanBooksCollective ? { label: "African Books Collective", href: book.africanBooksCollective } :
    book.cassavaRepublic ? { label: "Cassava Republic", href: book.cassavaRepublic } :
    book.booknookGhana ? { label: "Booknook Ghana", href: book.booknookGhana } :
    book.cityReads ? { label: "City Reads", href: book.cityReads } :
    null;
...
function fireConfetti() {
  ...
  const colors = ["#2d6a4f", "#40916c", "#d4a373", "#e9c46a", "#f4a261", "#e76f51", "#264653"];
  ...
  function draw() {
    ...
    for (const p of particles) {
      ...
      p.y += p.vy;
      p.rot += p.vr;
      if (t > 90) p.opacity = Math.max(0, p.opacity - 0.012);
      if (p.y < canvas.height + 10 && p.opacity > 0) alive = true;
      ctx.save();
...
function Toolbar({ theme, onTheme, soundEnabled, onSound, musicEnabled, onMusic, onAbout, onRecommend }) {
  const [pickerOpen, setPickerOpen] = useState(false);
  const [menuOpen, setMenuOpen] = useState(false);
  const isMobile = useIsMobile();

  function closeMenu() { setMenuOpen(false); setPickerOpen(false); }
        ...
        <div style={{ position: "relative" }}>
          ...
          {pickerOpen && (
            <div style={s.colorPicker}>
              {THEMES.map(t => (
                <button key={t.id} type="button" title={t.label} aria-label={t.label} aria-pressed={theme === t.id}
                  onClick={() => { onTheme(t.id); setPickerOpen(false); }}
                  style={{ ...s.colorSwatch, background: t.bg, border: theme === t.id ? "2px solid var(--text)" : "2px solid var(--border-2)" }}
                />
              ))}
            </div>
          )}
  ...
  const pillBase = {
    ...
    animation: "pillFadeUp 0.18s ease both",
  };
  ...
  return (
    <>
      ...
      {menuOpen && (
        <div onClick={closeMenu} style={{ position: "fixed", inset: 0, zIndex: 48 }} aria-hidden="true" />
      )}

      {/* Dots + expandable menu — bottom right */}
      <div style={{ position: "fixed", bottom: 24, right: 24, zIndex: 49, display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 12 }}>
        {menuOpen && (
          <>
            {/* Recommend */}
            <button type="button" className="toolbar-btn-pill" style={pillBase}
              onClick={() => { closeMenu(); onRecommend(); }}>
              <SvgRecommend /> Recommend a book
            </button>
            {/* About */}
            <button type="button" className="toolbar-btn-pill" style={{ ...pillBase, animationDelay: "0.06s" }}
              onClick={() => { closeMenu(); onAbout(); }}>
              <SvgAbout /> About this archive
            </button>
            {/* Theme */}
            <div style={{ position: "relative" }}>
              {pickerOpen && (
                <div style={{ ...s.colorPicker, bottom: 48, right: 0, zIndex: 60 }}>
                  {THEMES.map(t => (
                    <button key={t.id} type="button" title={t.label} aria-label={t.label} aria-pressed={theme === t.id}
                      onClick={() => { onTheme(t.id); closeMenu(); }}
                      style={{ ...s.colorSwatch, background: t.bg, border: theme === t.id ? "2px solid var(--text)" : "2px solid var(--border-2)" }}
                    />
                  ))}
                </div>
              )}
              <button type="button" className="toolbar-btn-pill" style={{ ...pillBase, animationDelay: "0.12s" }}
                onClick={() => setPickerOpen(v => !v)} aria-expanded={pickerOpen}>
        ...
        {/* Dots toggle button */}
        <button type="button" className="toolbar-btn" style={{ ...s.toolbarBtn, zIndex: 50 }}
...
export default function NigerianLit() {
  ...
  const [musicEnabled, setMusicEnabled] = useState(() => readPref(MUSIC_KEY, true));
  ...
  // The YouTube API is loaded only when music is actually wanted, so visitors
  // who never enter the archive — or who have music off — are not handed
  // YouTube cookies on arrival.
  function loadYTApi() {
  ...
  function handleEnter() {
    setShowSplash(false);
    if (musicEnabled) initYTPlayer();
  }

  // Interface sounds and the playlist are separate: page turns and stamps can
  // stay on with the music off, and both choices survive a reload.
  ...
  return (
    <div style={s.page}>
      ...
      <Toolbar theme={theme} onTheme={setTheme} soundEnabled={soundEnabled} onSound={toggleSound} musicEnabled={musicEnabled} onMusic={toggleMusic} onAbout={() => setShowAbout(true)} onRecommend={() => setShowRecommend(true)} />
...
const css = `
  ...
  * { box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
  ...
  .toolbar-btn-pill:hover { background: var(--surface-2) !important; }
