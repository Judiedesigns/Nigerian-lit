    ...
    return (
      <div style={s.toolbar}>
        ...
        <button type="button" className="toolbar-btn" style={s.toolbarBtn} onClick={onSound}
          ...
          aria-label={soundEnabled ? "Mute sound" : "Unmute sound"} aria-pressed={soundEnabled}>
  ...
  return (
    <>
      ...
      <div style={{ position: "fixed", bottom: 24, right: 24, zIndex: 49, display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 12 }}>
        ...
        {/* Bottom row: sound + dots always visible */}
        <div style={{ display: "flex", gap: 8 }}>
          <button type="button" className="toolbar-btn" style={s.toolbarBtn}
            onClick={onSound} aria-label={soundEnabled ? "Mute sound" : "Unmute sound"} aria-pressed={soundEnabled}>
            {soundEnabled ? <SvgSoundOn /> : <SvgSoundOff />}
          </button>
...
export default function NigerianLit() {
  ...
  function handleEnter() {
    setShowSplash(false);
    if (soundEnabled) initYTPlayer();
  }

  // One control for everything audible — the playlist and the interface sounds.
  // The choice is stored, so a visitor who asks for quiet stays quiet on return
  // and the YouTube player is never created for them.
